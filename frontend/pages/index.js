  ...
  return (
    <div>
      ...
      <div className="rounded-lg   bg-gray-900  text-white p-8 py-6 lg:w-3/6 t w-full  ">
          ...
          <iframe
            src={`https://carbon.now.sh/embed?bg=rgba%28171%2C+184%2C+195%2C+1%29&t=seti&wt=sharp&l=auto&width=680&ds=false&dsyoff=20px&dsblur=68px&wc=false&wa=true&pv=0px&ph=0px&ln=false&fl=1&fm=JetBrains+Mono&fs=14.5px&lh=163%25&si=false&es=2x&wm=false&code=${currentData.code}`}
            sandbox="allow-scripts allow-same-origin"
            className="w-full h-96 outline-none"
          ></iframe>
          <div className="w-full flex flex-row justify-evenly mt-6">
            <button   className="bg-blue-600 w-fit p-8 flex flex-row items-center justify-center  cursor-pointer text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 shadow-lg shadow-blue-500/50 dark:shadow-lg dark:shadow-blue-800/80 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2 ">
              <ThumbUpIcon />
              <p className="ml-3 font-mono font-bold">{currentData.likes.length }</p>
            </button>
            <button className="bg-gray-600 w-fit p-8 flex flex-row items-center justify-center  cursor-pointer  text-white bg-gradient-to-r from-gray-500 via-gray-600 to-gray-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-gray-300 dark:focus:ring-gray-800 shadow-lg shadow-gray-500/50 dark:shadow-lg dark:shadow-gray-800/80 font-medium rounded-lg text-sm px-5 py-4 text-center mr-2 mb-2">
              <ThumbDownIcon />
              <p className="ml-3 font-mono font-bold">{currentData.dislikes.length}</p>
            </button>
          </div>
          <div className="w-full mt-8 bg-gray-200 rounded-full h-2.5 dark:bg-gray-600">
            <div
              ...
              style={{ width: `${(currentData.likes.length / currentData.dislikes.length) * 100}%`, maxWidth: "100%" }}
            ></div>
          </div>
          <h1 className="w-full flex flex-row items-center justify-center text-blue-500 font-mono mt-6">Post Id: {currentData._id}</h1>
